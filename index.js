...
function loadHeadTable() { // загружаем заголовки столбцов таблицы
    ...
    <tr>
        ${checkColumnHead()}
    </tr>`; // создает шапку с заголовоками и проверяем есть ли он в списке тех заголовков которые должны отобразиться
}
...
function createDataAfterSort() { // создаем данные после сортировки
    ...
    for (person of personsData) { // перебераем элементы в массиве personsData
...
function check(person) { // проверяем какие заголовки и значение были выбраны
    if (sort_data.id) { // если таблица сортировалась по столбку id
        if (sort_data.id.indexOf(person.id) == -1) { // в данном столбце выбраного значение нет
            return false;
        }
    }
    if (sort_data.firstName) { // если таблица сортировалась по столбку firstName и т.д.
    ...
    return true; // если по столбцам сортировак не было или значение было найдено
}
...
function checkColumnBody(person) { // проверяем столбцы и создаем элементы строки
    ...
    if (eyeColor_column.checked) {
        ans += `<td class="other"><div class="eyeColor">${person.eyeColor}<div style="background-color: ${person.eyeColor}"></div></div></td>`; // в этом элементе создаем DIV у которого в стилях присваеваем значение цвета
    }
...
function createPageList() { // рассчитаем сколько страниц выбранном способе вывода данных будет и на какой страницы находится таблица
    ...
    if (maxRow < personsDataAfterSort.length) { // если способ вывода данных меньше длины таблицы
        pages.style.display = 'flex'; // отображаем интерфей перемежения по страницам
...
function modeShowData(value = maxRow) { // устанавливаем параметры отображения данных
    firstRow = 0; // индекс первого эдемента
    pageCount = 1; // номер страцы
    maxRow = value; // максимальное кол-во элементов на странице
    lastRow = maxRow; // последний элемент страцы
...
function sortTargetTable() { // создаем массив сортированных данных по значению
    ...
    for (child of sort_list.childNodes) { // перебираем список элементов
        if (child.tagName == "DIV" && child.childNodes[1].checked) { // находим checkbox и проверяем постален ли в нем флажек
...
function formDataOpen() { // загружаем форму редактирования данных
    ...
    if (idPersonData) { // если ранее строка таблицы была выбрано т.е. по ней был клик то в поле input выводяться значения строки
